	...
	char **error)
{
	...
	s64 actual_usecs = live_time_to_script_time_usecs(
        ...
        //CHANGE don't want this for approximate networking
	...
	if (map_outbound_live_packet(socket, live_packet, actual_packet, script_packet, error))
		goto out;
	...
	if (script_packet->tcp) {
		...
		if (verify_outbound_live_tcp_options(state->config, actual_packet, script_packet, error)) {
			non_fatal = true;
			goto out;
		}
	}
	...
	if (verify_time(state, time_type, script_usecs,script_usecs_end, live_packet->time_usecs, "outbound packet", error)) {
		non_fatal = true;
		goto out;
	}
